    ...
    */
    public function itemAttributeAngularAdd(Request $request)
    {
        ...
        //    return ['success'=> true,'message' => 'Item Attribute has been added.'];
            ...
            Session::flash('message','Item Attribute has been added.');
            return redirect('itemattribute');
  ...
  /**
    ...
    */
    public function itemAttributeAngularEdit(Request $request)
    {
        ...
        $itemAttributeId = 0;
        $itemAttributeId = $request->input('ItemAttId');
        ...
        if (empty($resultName)) { 
            ...
            Session::flash('message','Item Attribute has been updated.');
            return redirect('itemattribute');
            ...
            $getItemAttDataByID = [];
            $itemAttributeModel = new ItemAttributeModel();
            $getItemAttDataByID = $itemAttributeModel->selectItemAttributeDataByID($itemAttributeId);
            
            
            return view('admin.itemattribute', [
                'nameText' => $nameText,
                'getItemAttDataByID' => $getItemAttDataByID
            ]);
     ...
     * @author <satishchauhan041@gmail.com>
     */
    public function getAngularItemAttByID($processId = null)
    {   
        $processIds = base64_decode($processId);
        if (!empty($processIds)) {
            $itemAttributeModel = new ItemAttributeModel();
            $getProcessDataByID = $itemAttributeModel->selectItemAttributeDataByID($processIds);
            
        }
        return view('admin.itemattribute', [
            'getItemAttDataByID' => $getProcessDataByID, 
        ]);
   ...
   /**
     ...
     * @author <satishchauhan041@gmail.com>
     */
    public function removeAngularItemAttById($processId = null)
    {
       
       // $itemAttributeId = $request->input('id');
        
        $itemAttributeId = base64_decode($processId);
   ...
   /**
    ...
    public function getItemNamebyPartyId()
    {
        ...
        if (!empty($_GET['partyIds'])) {
            ...
            $getItemNameData = 
            DB::select('SELECT * FROM item_master WHERE is_deleted_status = "N" 
            AND party_id = '.$_GET['partyIds'].' ORDER BY name ASC');
...
*/
  ...
  /**
    ...
    */
    ...
    public function getPoReports($poId = 0)
    {   
                ...
                foreach ($purchase_ordersArr as $key => $itemData) {
                    ...
                    if (!empty($itemUnitData)) {
                        foreach ($itemUnitData as  $unitType) {
                            ...
                            $purchase_ordersArr[$key]->unitId = !empty($unitType->unit_id) ? $unitType->unit_id : 0;
                            $unitId = !empty($unitType->unit_id) ? $unitType->unit_id : 0;
      ...
      $purchase_ordersArr = [];
        ...
        if (!empty($poIds)) {
            ...
            $itemIdNew  = !empty($purchase_ordersArr[0]->item_id) ? $purchase_ordersArr[0]->item_id : 0; 
            ...
            DB::select('SELECT * FROM item_attribute_value WHERE 
...
DB::table('item_attribute_master')
  ->select('item_attribute_master.id','item_attribute_master.name','item_attribute_value.value')
  ->join('item_attribute_value','item_attribute_value.attribute_item_id','=','item_attribute_master.id')
  ->where('item_attribute_master.is_deleted_status', 'N')
  ->where('item_attribute_value.is_deleted_status', 'N')
/*  ->whereIN('item_attribute_value.attribute_item_id', $attribute_item_id)
*/

  ->where('item_attribute_value.item_id', $itemData->item_id)
//->where('purchase_order_attribute.po_id', $itemData->id)
->orderBy('item_attribute_master.name', 'ASC')
->get()
->toArray();
...
*/
                ...
                AND  id IN('.$attItemIdlist.') ORDER By name ASC ');
            ...
            /*$pdf = PDF::loadView('admin.attendanceReports',[
                'purchase_orders' => $purchase_ordersArr,
                'attribute_item_id_values'        => !empty($finDATA) ? $finDATA : 0,
                'attribute_item_name'        => $attribute_item_name,
            ...
            $reportFinalDate =  time().'_'.$itemData->po_no.'_'.date('d_m_Y').'_purchase_order_invoice.pdf';
            $pdf->save('laravel/storage/purchase_order_invoice/'.$reportFinalDate.'');
            return  $pdf->download($reportFinalDate);  
            */
...
*/
  ...
  /**
    ...
    * @author <satishchauhan041@gmail.com>
    */
    public function jobCardAngularAdd(Request $request)
    {   
        
        $po_id  = 
        !empty($request->input('po_id')) ? $request->input('po_id') : 0;
        
        $item_idArr  = !empty($request->input('item_id_'.$po_id.'')) ? $request->input('item_id_'.$po_id.'') : 0;   
            
        $process_idArr  = !empty($request->input('process_id_'.$po_id.'')) ? $request->input('process_id_'.$po_id.'') : 0;   
       
        $noteArrs  = !empty($request->input('notes_'.$po_id.'')) ? $request->input('notes_'.$po_id.'') : 0;   

        $job_card_nos  = !empty($request->input('job_card_no_'.$po_id.'')) ? $request->input('job_card_no_'.$po_id.'') : 0;   

        $job_card_dates  = !empty($request->input('job_card_date_'.$po_id.'')) ? $request->input('job_card_date_'.$po_id.'') : 0; 

        $machineIdArr  = !empty($request->input('machineId_'.$po_id.'')) ? $request->input('machineId_'.$po_id.'') : 0;   
        $item_cat_ids  = !empty($request->input('item_category_id_'.$po_id.'')) ? $request->input('item_category_id_'.$po_id.'') : 0;   
       
        $process_dateArr  = !empty($request->input('process_date_'.$po_id.'')) ? $request->input('process_date_'.$po_id.'') : 0;   
        ...
        $jobCardData = [ 
            ...
            'po_id'            => !empty($po_id) ? $po_id : 0,
            'item_category_id' => !empty($item_cat_ids) ? $item_cat_ids : 0 ,
            'item_id'          => !empty($item_idArr) ? $item_idArr : 0,
            'job_card_no'      => !empty($job_card_nos) ? $job_card_nos : 0,
            'job_card_date'    => !empty($job_card_dates) ? $job_card_dates : 0,
            'status'           => $status,
            'created_at'       => $time,
            'updated_at'       => $time,
            'deleted_at'       => $time,
        ...
        $deleteResults = DB::table('job_card')->where('po_id', $po_id)->update($purchaseDatasss);

        if (!empty(array_sum($process_idArr))) {

            $insertJobCardRes = 
            DB::table('job_card')->insertGetId($jobCardData);
            ...
            if (!empty($insertJobCardRes)) {    
                ...
                foreach ($process_idArr as $key => $itemAtts) { 
                    ...
                    $itemAttsArrs = [
                        ...
                        'machine_id'        => !empty($machineIdArr[$key]) ? $machineIdArr[$key] : 0,
                    ...
                    $insertResult = 
            DB::table('job_card_process_list')->insertGetId($itemAttsArrs);
   ...
   /**
    ...
    /*public function removeAngularItemAttById($processId = null)
    {
        ...
        if (!empty($processIds)) {
            ...
            $purhaseOrderModel = new PurhaseOrderModel();
            $result = $purhaseOrderModel->deleteProcess($processIds);
