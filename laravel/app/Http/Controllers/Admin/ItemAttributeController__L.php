    ...
    */
    ...
    {   
        ...
        if (!empty($poIds)) {
            ...
            DB::select('SELECT * FROM attribute_item_master WHERE is_deleted_status = "N" AND item_id = '.$itemData->item_id.'');
...
*/         
        ...
        if (!empty($purchase_ordersArr)) {
            foreach ($purchase_ordersArr as $poData) {
                ...
                $pdf->cell(185, 6, 'Party Name : '.$poData->partyName.'', 1, 0, 'L', '');
                $pdf->cell(39, 6, 'PO.No : '.$poData->po_no.'', 1, 0, 'L', '');
                $pdf->cell(50, 6, 'PO Date : '.$poData->po_date, 1, 0, 'L', '');
                ...
                $pdf->row(array( , ''.$poData->unitName.'', ''.number_format($poData->qty,2).''), 
                    array('L', 'L', 'L'), '', '', true, 5);
